...
#위상정렬 # 사이클이없는 방향그래프에서 노드 순서를 찾는 알고리즘
#다익스트라 # 최단거리알고리즘 # 시작점이있고 다른 모든 노드로 가는 최단거리를 구하는 알고리즘 *음수간선X
#벨만포드 # 최단거리알고리즘 # 음수사이클있는지 체크하는 문제 ex) 시간여행, 웜홀 등등 *음수간선O
#플로이드워셜 # 최단거리알고리즘 # 시작점X, 시간복잡도 안좋음 , 모든도시를 연결하는 최단거리 (N이 작아야함)
#최소신장트리 # 모든 노드를 연결할때 사용된 에지들의 가중치의 합을 최소로 하는 트리
##################################################################################################################################
#트리 세크먼트 트리 # 구간합구하기3
#트리 최소공통조상 구하기
##################################################################################################################################
...
##################################################################################################################################
##################################################################################################################################
# 최단거리알고리즘 # 시작점이있고 다른 모든 노드로 가는 최단거리를 구하는 알고리즘 *음수간선X
#1753
import sys
input = sys.stdin.readline
...
##################################################################################################################################
##################################################################################################################################
# 최단거리알고리즘 # 음수사이클있는지 체크하는 문제 ex) 시간여행, 웜홀 등등 *음수간선O
#11657
import sys
input = sys.stdin.readline
N, M = map(int, input().split())
...
##################################################################################################################################
##################################################################################################################################
# 최단거리알고리즘 # 시작점X, 시간복잡도 안좋음 , 모든도시를 연결하는 최단거리 (N이 작아야함)
#11404
import sys
input = sys.stdin.readline
N = int(input())
